...
class ProjectEulerSpecs extends Specification {

  val testClassInstance = ProjectEuler
  val testClass = testClassInstance.getClass
  ...
  def methodName( p:Int ) = "problem" + (10000+p).toString.takeRight(3)

  val attemptedSolutions: scala.collection.immutable.SortedMap[Int, String] = (for (
    (prob, answer) <- allProblems;
    method = methodName(prob);
    attempt <- Try(ProjectEuler.getClass.getMethod(method)).toOption
    if attempt.getReturnType.getSimpleName != "void"
  ) yield prob -> testClass.getMethod(method).invoke(testClassInstance).toString)(breakOut)

  val score = (attemptedSolutions.count { case (p, answer) => answer == allProblems(p) }, attemptedSolutions.size)

  val problems = attemptedSolutions.map(t => t._1 -> allProblems(t._1))
  val answers = attemptedSolutions.map(t => t._1 -> t._2)

  val summary =
    if(attemptedSolutions.nonEmpty)
      100 * score._1 / score._2 + "%  (" + score._1 + " correct of " + score._2 + " tried)"
    else
      "No problems have been attempted"


  def resultText( p:Int ) = methodName(p) + " = " + answers(p) + (if(problems(p) == answers(p))" is CORRECT" else "")

  class problemSpec( p:Int ) extends Specification { def is = resultText(p) ! {problems(p) mustEqual answers(p) } }

  // Chain together all the tests in one monster test
  //
  //    (Warning: This technique will blow the stack eventually, but for the number of tests here it works.)
  //
  def is = answers.keySet.foldLeft(summary.title ^ p) { case (chain,prob) => chain ^ new problemSpec(prob) }
}